...
const MODES=[{id:'normal',name:'NORMAL',mult:1.2,desc:'A ameaça original.'},{id:'survival',name:'SOBREVIVÊNCIA',mult:1.9,desc:'Hordas maiores e mais resistentes.'},{id:'madness',name:'LOUCURA',mult:3,desc:'Inimigos velozes e brutais.'},{id:'hardcore',name:'HARDCORE',mult:5,desc:'Sem misericórdia. Recompensas máximas.'}];
...
function difficulty(state){ensure(state);let c=state.campaign,mode=MODES[c.mode];return mode.mult*(1+(c.map-1)*.42+(c.level-1)*.1+c.cycles*.4)}
