...
function category(item){if(item?.archetype==='iris')return'iris';if(item?.archetype==='healer')return'healer';if(item?.archetype==='maya')return'dps';if(item?.archetype==='bruno')return'tank';let s=item?.stats||{},scores={dps:Number(s.attack||0)*10+Number(s.rate||0)*220+Number(s.crit||0)*180,tank:Number(s.hp||0)*.85,healer:Number(s.healing||0)*280+Number(s.rate||0)*65+Number(s.scrap||0)*45};return Object.entries(scores).sort((a,b)=>b[1]-a[1])[0][0]}
function rank(items,heroId,slot){return(items||[]).filter(i=>!slot||i.slot===slot).map(item=>({item,score:score(item,heroId)})).sort((a,b)=>b.score-a.score)}
function bestLoadout(items,heroId){return['weapon','armor','charm'].reduce((set,slot)=>{set[slot]=rank(items,heroId,slot)[0]?.item||null;return set},{})}
return{WEIGHTS,SCALE,score,bestHero,compare,suitability,category,rank,bestLoadout};
});
