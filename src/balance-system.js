...
function validLevel(level){let value=Number(level);return Number.isFinite(value)?Math.max(1,Math.floor(value)):1}
function expectedHero(id,level=1){let base=HERO_BASE[id]||HERO_BASE.maya,growth=GROWTH[id]||GROWTH.maya,l=validLevel(level);return{maxHp:Math.round(base[0]+(l-1)*growth[0]),attack:round(base[1]+(l-1)*growth[1],1),rate:base[2]}}
function normalizeHero(hero){hero.level=validLevel(hero.level);let expected=expectedHero(hero.id,hero.level);hero.maxHp=expected.maxHp;hero.atk=expected.attack;hero.rate=expected.rate;hero.hp=Math.min(expected.maxHp,Math.max(0,Number(hero.hp||expected.maxHp)));return hero}
